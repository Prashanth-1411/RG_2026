...
@section('page-title', 'Edit Featured Section')

@section('content')
<div class="bg-white rounded-xl border border-gray-200 p-6 max-w-3xl">
        <form action="{{ route('admin.featured_sections.update', $item) }}" method="POST" enctype="multipart/form-data">
            @csrf @method('PUT')
            <div class="space-y-5">
            <div class="mb-4">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="section_type" value="{{ old('section_type', $item->section_type) }}" class="w-full px-4 py-2.5 rounded-lg border border-gray-300 text-sm focus:ring-2 focus:border-transparent outline-none transition-all @error('section_type') border-red-500 @enderror">
