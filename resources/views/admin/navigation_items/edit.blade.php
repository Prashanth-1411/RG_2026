...
<div class="bg-white rounded-xl border border-gray-200 p-6 max-w-3xl">
    <form action="{{ route('admin.navigation_items.update', $item) }}" method="POST" enctype="multipart/form-data">
        @csrf @method('PUT')
        <div class="space-y-5">
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-1.5">Label</label>
                <input type="text" name="label" value="{{ old('label', $item->label) }}" class="w-full px-4 py-2.5 rounded-lg border border-gray-300 text-sm focus:ring-2 focus:border-transparent outline-none transition-all @error('label') border-red-500 @enderror">
            ...
            <div>
                ...
                <input type="text" name="link" value="{{ old('link', $item->link) }}" class="w-full px-4 py-2.5 rounded-lg border border-gray-300 text-sm focus:ring-2 focus:border-transparent outline-none transition-all @error('link') border-red-500 @enderror">
            ...
            <div>
                ...
                <input type="number" name="parent_id" value="{{ old('parent_id', $item->parent_id) }}" class="w-full px-4 py-2.5 rounded-lg border border-gray-300 text-sm focus:ring-2 focus:border-transparent outline-none transition-all @error('parent_id') border-red-500 @enderror">
            ...
            <div>
                ...
                <select name="location" class="w-full px-4 py-2.5 rounded-lg border border-gray-300 text-sm focus:ring-2 focus:border-transparent outline-none transition-all @error('location') border-red-500 @enderror">
                    ...
                    <option value="{{ $opt }}" {{ old('location', $item->location) == $opt ? 'selected' : '' }}>{{ ucfirst($opt) }}</option>
                    @endforeach
                </select>
