...
@section('page-title', 'Manage Gallery Images')
@section('content')
<div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
    <div class="flex items-center justify-between p-5 border-b border-gray-100">
        <h5 class="text-sm font-bold text-gray-900">All Gallery Images</h5>
        <a href="{{ route('admin.gallery_images.create') }}" class="inline-flex items-center gap-2 px-4 py-2 text-white text-sm font-semibold rounded-lg transition-colors" style="background: var(--theme-primary);">
                ...
                <tr class="hover:bg-gray-50 transition-colors">
                    ...
                    <td class="px-5 py-3 text-right">
                        <div class="flex items-center gap-1 justify-end">
                            <a href="{{ route('admin.gallery_images.show', $item) }}" class="p-1.5 rounded-lg text-cyan-600 hover:bg-cyan-50 transition-colors" title="View"><i class="ti ti-eye"></i></a>
                            <a href="{{ route('admin.gallery_images.edit', $item) }}" class="p-1.5 rounded-lg text-amber-600 hover:bg-amber-50 transition-colors" title="Edit"><i class="ti ti-pencil"></i></a>
                            <form action="{{ route('admin.gallery_images.destroy', $item) }}" method="POST" onsubmit="return confirm('Are you sure?')" class="inline">
                                @csrf @method('DELETE')
