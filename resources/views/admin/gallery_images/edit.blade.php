...
<div class="bg-white rounded-xl border border-gray-200 p-6 max-w-3xl">
    <form action="{{ route('admin.gallery_images.update', $item) }}" method="POST" enctype="multipart/form-data">
        @csrf @method('PUT')
        <div class="space-y-5">
            <div>
                ...
                <select name="album_id" class="w-full px-4 py-2.5 rounded-lg border border-gray-300 text-sm focus:ring-2 focus:border-transparent outline-none transition-all @error('album_id') border-red-500 @enderror">
                    ...
                    <option value="{{ $val }}" {{ old('album_id', $item->album_id) == $val ? 'selected' : '' }}>{{ $opt }}</option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <input type="text" name="alt_text" value="{{ old('alt_text', $item->alt_text) }}" class="w-full px-4 py-2.5 rounded-lg border border-gray-300 text-sm focus:ring-2 focus:border-transparent outline-none transition-all @error('alt_text') border-red-500 @enderror">
