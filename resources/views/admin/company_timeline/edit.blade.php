...
@section('title', 'Edit Company Timeline Entry')
@section('page-title', 'Edit Company Timeline Entry')

@section('content')
<div class="bg-white rounded-xl border border-gray-200 p-6 max-w-3xl">
    <form action="{{ route('admin.company_timeline.update', $item) }}" method="POST" enctype="multipart/form-data">
        @csrf @method('PUT')
        <div class="space-y-5">
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-1.5">Year</label>
                <input type="text" name="year" value="{{ old('year', $item->year) }}" class="w-full px-4 py-2.5 rounded-lg border border-gray-300 text-sm focus:ring-2 focus:border-transparent outline-none transition-all @error('year') border-red-500 @enderror">
        ...
        <div class="flex items-center gap-3 pt-5 border-t border-gray-100 mt-6">
            ...
            <a href="{{ route('admin.company_timeline.index') }}" class="px-6 py-2.5 text-gray-700 text-sm font-semibold rounded-lg border border-gray-300 hover:bg-gray-50 transition-colors">Cancel</a>
        </div>
    </form>
</div>
@endsection
