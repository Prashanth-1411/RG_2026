    ...
    <form action="{{ route('admin.certificates.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="space-y-5">
            ...
            <div>
                ...
                <input type="text" name="issuer" value="{{ old('issuer') }}" class="w-full px-4 py-2.5 rounded-lg border border-gray-300 text-sm focus:ring-2 focus:border-transparent outline-none transition-all @error('issuer') border-red-500 @enderror">
            ...
            <div>
                ...
                <input type="date" name="date_issued" value="{{ old('date_issued') }}" class="w-full px-4 py-2.5 rounded-lg border border-gray-300 text-sm focus:ring-2 focus:border-transparent outline-none transition-all @error('date_issued') border-red-500 @enderror">
            ...
            <div>
                ...
                <select name="type" class="w-full px-4 py-2.5 rounded-lg border border-gray-300 text-sm focus:ring-2 focus:border-transparent outline-none transition-all @error('type') border-red-500 @enderror">
                    ...
                    <option value="{{ $opt }}" {{ old('type') == $opt ? 'selected' : '' }}>{{ ucfirst($opt) }}</option>
                    @endforeach
                </select>
