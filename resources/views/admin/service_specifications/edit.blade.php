...
@section('title', 'Edit Service Specification')
@section('page-title', 'Edit Service Specification')

@section('content')
<div class="bg-white rounded-xl border border-gray-200 p-6 max-w-3xl">
    <form action="{{ route('admin.service_specifications.update', $item) }}" method="POST" enctype="multipart/form-data">
        @csrf @method('PUT')
        <div class="space-y-5">
            <div>
            ...
            <div>
                ...
                <input type="text" name="spec_key" value="{{ old('spec_key', $item->spec_key) }}" class="w-full px-4 py-2.5 rounded-lg border border-gray-300 text-sm focus:ring-2 focus:border-transparent outline-none transition-all @error('spec_key') border-red-500 @enderror">
            ...
            <div>
                ...
                <input type="text" name="spec_value" value="{{ old('spec_value', $item->spec_value) }}" class="w-full px-4 py-2.5 rounded-lg border border-gray-300 text-sm focus:ring-2 focus:border-transparent outline-none transition-all @error('spec_value') border-red-500 @enderror">
