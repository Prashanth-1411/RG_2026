...
import {
  ShieldCheck, ArrowRight, Heart, X, Phone, Calendar, Clock,
  MapPin, Users, Star, CheckCircle2, Award, HeartPulse, Sparkles,
  Search, Filter, Church, BookOpen, Truck, Send
} from 'lucide-react';
...
import { funeralServices as staticFuneralServices } from '../data/funeral-services';
import fallbackImg from '../assets/funeral-1.jpg';
...
export const FuneralServices: React.FC = () => {
  const [selectedService, setSelectedService] = useState<typeof staticFuneralServices[number] | null>(null);
  const [activeFilter, setActiveFilter] = useState<string>('all');
  const [funeralServices, setFuneralServices] = useState(staticFuneralServices);
  const [pageHeading, setPageHeading] = useState('Dignified Funeral Care');
  const [pageContent, setPageContent] = useState('Compassionate homage services for every family.');
  ...
  useEffect(() => {
    ...
    getServices().then((items: ServiceItem[]) => {
      const active = items.filter((s) => (s.status === 'active' || s.is_active) && s.service_type === 'funeral');
      if (active.length) {
        setFuneralServices(active.map((s) => ({
          id: String(s.id),
          title: s.title,
          ...
          features: [],
          price: '',
        })) as unknown as typeof staticFuneralServices);
      }
    });
  }, []);
  ...
  const categories = [
    ...
    { id: 'ceremony', label: 'Ceremony', icon: <Church className="w-3.5 h-3.5" /> },
    { id: 'support', label: 'Support', icon: <BookOpen className="w-3.5 h-3.5" /> },
  ];

  const getServiceCategory = (s: typeof funeralServices[number]) => {
    const transport = ['Funeral Van', 'Freezer Box', 'Dead Body Transport'];
    const ceremony = ['Coffin Lowering', 'VIP Funeral', 'Casket', 'Religious'];
    if (transport.some(e => s.title.includes(e))) return 'transport';
    if (ceremony.some(c => s.title.includes(c))) return 'ceremony';
    return 'support';
  };

  const filteredServices = activeFilter === 'all'
    ? funeralServices
    : funeralServices.filter(s => getServiceCategory(s) === activeFilter);
  ...
  return (
    <div className="pt-20">
      {/* ========== CINEMATIC HERO ========== */}
      <ParallaxSection>
        <section className="relative min-h-[70vh] flex items-center overflow-hidden bg-navy-900">
        <FloatingShape className="w-72 h-72 bg-gold-500 top-10 -left-20" delay={0} />
        <FloatingShape className="w-96 h-96 bg-brand-500 bottom-20 -right-20" delay={2} />
        ...
        <div className="absolute inset-0 overflow-hidden">
          ...
          <div className="absolute top-0 left-3/4 w-px h-full bg-gradient-to-b from-transparent via-gold-500/10 to-transparent" />
        </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center py-20">
            ...
            >
              ...
              <h1 className="text-5xl sm:text-6xl lg:text-7xl font-black text-white font-display leading-[1.05] tracking-tight">
                Dignified Funeral
                <span className="block text-gradient"> & Homage Services</span>
              </h1>

              <p className="text-lg text-navy-300 leading-relaxed max-w-xl font-body">
                AC hearses, deceased preservation, VIP ceremonial arrangements, and full legal assistance. 
                Serving families with absolute respect, dignity, and compassion across all communities.
              </p>
              ...
              >
                ...
                <div className="flex items-center gap-2 text-sm text-navy-400">
                  ...
                  <span>Govt. Approved Mortuary</span>
                </div>
                <div className="flex items-center gap-2 text-sm text-navy-400">
                  <Heart className="w-4 h-4 text-gold-400 fill-gold-400" />
                  <span>2,000+ Families Served</span>
                </div>
              </motion.div>
            </motion.div>
            ...
            >
              <div className="relative">
                ...
                <div className="relative rounded-3xl overflow-hidden border border-white/10 shadow-2xl">
                  <BackgroundImage
                    src={getImage(funeralServices[0]?.image_path)}
                    alt="Funeral Hearse Van"
                    className="h-[500px]"
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-2xl font-black text-white font-display">2,000+</p>
                      <p className="text-xs text-navy-400">Families Served with Dignity</p>
                    </div>
                  </div>
                </motion.div>
              </div>
        ...
        >
          <div className="w-6 h-10 rounded-full border-2 border-white/20 flex items-start justify-center p-1.5">
            <motion.div className="w-1.5 h-3 rounded-full bg-gold-400" />
          </div>
        </motion.div>
      </section>
      </ParallaxSection>
      ...
      <section className="relative py-12 bg-white border-b border-navy-100/50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-8">
            <AnimatedCounter end={12} label="Years of Service" suffix="+" icon={<Award className="w-6 h-6 text-brand-500" />} />
            <AnimatedCounter end={2000} label="Families Served" suffix="+" icon={<Heart className="w-6 h-6 text-gold-500" />} />
            <AnimatedCounter end={8} label="Funeral Vehicles" suffix="" icon={<Truck className="w-6 h-6 text-brand-500" />} />
            <AnimatedCounter end={100} label="Cities Covered" suffix="+" icon={<MapPin className="w-6 h-6 text-brand-500" />} />
          </div>
        </div>
      </section>
      ...
      <section className="py-12 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <AnimatedSection spring>
            <SectionHeader
              title="Complete Funeral & Homage Services"
              subtitle="Eight specialized service categories — from AC hearse vans and deceased preservation to VIP ceremony arrangements and legal assistance."
            />
          </AnimatedSection>
          ...
          <div className="flex flex-col lg:flex-row items-center justify-between gap-8">
            <AnimatedSection direction="left">
              <div className="space-y-4 text-center lg:text-left">
                <span className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-gold-500/10 text-gold-400 text-[10px] uppercase font-bold tracking-widest border border-gold-500/20">
                  <span className="w-1.5 h-1.5 bg-gold-400 rounded-full animate-pulse" />
                  Immediate Funeral Assistance Needed?
                </span>
                ...
                <p className="text-base text-navy-300 max-w-xl font-body">
                  Our funeral coordinators are available 24/7. Call us for immediate assistance with arrangements, transportation, or guidance during this difficult time.
                </p>
              </div>
            </AnimatedSection>
            ...
            <AnimatedSection direction="right" className="flex flex-col sm:flex-row gap-4 w-full sm:w-auto">
              <Magnetic>
                <motion.a
                  ...
                  whileHover={{ scale: 1.03 }}
                  whileTap={{ scale: 0.97 }}
                  className="flex items-center justify-center gap-2.5 px-8 py-4 bg-white text-navy-800 font-black rounded-xl shadow-xl text-sm hover:bg-navy-50 transition-all"
                >
                  <Phone className="w-5 h-5" />
                  <span>Call Helpline: +91 95516 63530</span>
                </motion.a>
              </Magnetic>
              <Magnetic>
                <motion.a
                  href="https://wa.me/918778481556?text=I+need+funeral+arrangement+assistance"
                  target="_blank"
                  rel="noreferrer"
                  whileHover={{ scale: 1.03 }}
                  whileTap={{ scale: 0.97 }}
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <AnimatedSection>
            <div className="text-center mb-12">
              ...
              <p className="mt-4 text-navy-400 font-body text-sm leading-relaxed max-w-lg mx-auto">
                Submit the details below. Our funeral coordinator will contact you within minutes to discuss arrangements and provide compassionate support.
              </p>
            </div>
          </AnimatedSection>
          ...
          {bookingSuccess ? (
            ...
            >
              ...
              <h3 className="text-2xl font-bold text-white font-display">Arrangement Request Received</h3>
              <p className="text-navy-400 font-body text-sm">
                Our funeral coordinator will call you shortly to discuss the arrangements with compassion and care.
              </p>
              <Magnetic>
                <motion.button
                  whileHover={{ scale: 1.03 }}
            ...
            >
              <div className="space-y-2 field-focus">
                <label className="text-xs font-bold uppercase tracking-wider text-navy-400 font-body">Contact Person Name</label>
                <input type="text" required placeholder="Enter your full name" value={bookingForm.name} onChange={e => setBookingForm({...bookingForm, name: e.target.value})}
              ...
              <div className="space-y-2 field-focus">
                <label className="text-xs font-bold uppercase tracking-wider text-navy-400 font-body">Current Location</label>
                <input type="text" required placeholder="e.g. Anna Nagar, Chennai" value={bookingForm.pickup} onChange={e => setBookingForm({...bookingForm, pickup: e.target.value})}
                  className="w-full px-4 py-3.5 bg-white/5 border border-white/10 rounded-xl focus:border-brand-500 focus:outline-none text-white text-sm placeholder:text-navy-500 transition-colors" />
              </div>
              <div className="space-y-2 field-focus">
                <label className="text-xs font-bold uppercase tracking-wider text-navy-400 font-body">Destination / Venue</label>
                <input type="text" required placeholder="e.g. Besant Nagar Crematorium" value={bookingForm.destination} onChange={e => setBookingForm({...bookingForm, destination: e.target.value})}
                  className="w-full px-4 py-3.5 bg-white/5 border border-white/10 rounded-xl focus:border-brand-500 focus:outline-none text-white text-sm placeholder:text-navy-500 transition-colors" />
              </div>
              <div className="space-y-2 field-focus">
                <label className="text-xs font-bold uppercase tracking-wider text-navy-400 font-body">Select Service</label>
              ...
              <div className="space-y-2 field-focus">
                <label className="text-xs font-bold uppercase tracking-wider text-navy-400 font-body">Date of Service</label>
              ...
              <div className="md:col-span-2 space-y-2 field-focus">
                ...
                <input type="text" placeholder="Religious preferences, floral arrangements, specific rituals, etc." value={bookingForm.notes} onChange={e => setBookingForm({...bookingForm, notes: e.target.value})}
                  className="w-full px-4 py-3.5 bg-white/5 border border-white/10 rounded-xl focus:border-brand-500 focus:outline-none text-white text-sm placeholder:text-navy-500 transition-colors" />
              </div>
          ...
          >
            <div className="relative h-52 sm:h-64 overflow-hidden">
              ...
              <div className="absolute bottom-4 left-6 right-6">
                <div className="flex items-center gap-3">
                  <div className="w-12 h-12 rounded-xl bg-navy-800 flex items-center justify-center shadow-lg">
                    <Heart className="w-6 h-6 text-gold-400" />
                  </div>
                  <div>
                    <span className="text-xs text-gold-300 uppercase tracking-wider font-bold">Service Details</span>
            ...
            <div className="p-6 sm:p-8 space-y-6">
              ...
              <div className="space-y-3">
                <h4 className="text-xs uppercase font-extrabold tracking-widest text-brand-600 font-body">Standard Inclusions</h4>
              ...
              <div className="pt-4 flex gap-4">
                ...
                <motion.a
                  ...
                  whileTap={{ scale: 0.97 }}
                  href="tel:+919551663530"
                  className="px-6 py-3.5 bg-navy-800 hover:bg-navy-700 text-white font-bold rounded-xl text-sm transition-all duration-200 text-center shadow-lg flex items-center gap-2"
                >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
          ...
          >
            ...
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                <h2 className="text-xl font-bold text-navy-800 font-display">Arrange: {bookingForm.serviceName}</h2>
                <p className="text-xs text-navy-400 font-body">Fill the details below</p>
              </div>
            </div>
            ...
            {bookingSuccess ? (
              <div className="text-center py-8 space-y-3">
                ...
                <h3 className="text-lg font-bold text-navy-800 font-display">Request Submitted</h3>
                <p className="text-xs text-navy-400 font-body">A funeral coordinator will contact you shortly.</p>
              </div>
            ) : (
              <form onSubmit={handleBookingSubmit} className="space-y-4 font-body">
                <input type="text" required placeholder="Contact Person Name" value={bookingForm.name} onChange={e => setBookingForm({...bookingForm, name: e.target.value})}
                ...
                <input type="tel" required placeholder="Contact Phone" value={bookingForm.phone} onChange={e => setBookingForm({...bookingForm, phone: e.target.value})}
                  className="w-full px-4 py-2.5 border border-navy-200 rounded-xl focus:border-brand-500 focus:outline-none text-sm" />
                <input type="text" required placeholder="Current Location" value={bookingForm.pickup} onChange={e => setBookingForm({...bookingForm, pickup: e.target.value})}
                  className="w-full px-4 py-2.5 border border-navy-200 rounded-xl focus:border-brand-500 focus:outline-none text-sm" />
                <input type="text" required placeholder="Destination / Venue" value={bookingForm.destination} onChange={e => setBookingForm({...bookingForm, destination: e.target.value})}
                  className="w-full px-4 py-2.5 border border-navy-200 rounded-xl focus:border-brand-500 focus:outline-none text-sm" />
                <div className="grid grid-cols-2 gap-2">
                  <input type="date" value={bookingForm.date} onChange={e => setBookingForm({...bookingForm, date: e.target.value})}
                    className="w-full px-4 py-2.5 border border-navy-200 rounded-xl focus:border-brand-500 focus:outline-none text-sm" />
                  <input type="text" placeholder="Special Requests" value={bookingForm.notes} onChange={e => setBookingForm({...bookingForm, notes: e.target.value})}
                    className="w-full px-4 py-2.5 border border-navy-200 rounded-xl focus:border-brand-500 focus:outline-none text-sm" />
                </div>
                <motion.button
                  whileHover={{ scale: 1.02 }}
